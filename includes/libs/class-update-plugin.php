 ...
 *
 */
class CBC_Theme_Compatibility_Update_Plugin{
	/**
	 * Script version
	 * @var string - version
	 */
	 ...
	 */
	public function __construct( $update_url, $plugin_file, $request_params = array() ){
		$this->url = $update_url;
 ...
 *
 */
class CBC_Theme_Compatibility_Plugin_Update_Details{
	/**
	 * Script version
	 * @var string - version
	 */
	...
	private function request( $args ){
		...
		if ( is_wp_error( $request ) ) {
			$res = new WP_Error('cbc_theme_compatibility_plugins_api_failed', __( 'An unexpected error occurred. Something may be wrong with your server configuration. If you continue to have problems, please contact us.' ), $request->get_error_message() );
		} else {
			$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
			if ( ! is_object( $res ) && ! is_array( $res ) )
				$res = new WP_Error('cbc_theme_compatibility_plugins_api_failed', __( 'An unexpected error occurred. Something may be wrong with your server configuration. If you continue to have problems, please contact us.' ), wp_remote_retrieve_body( $request ) );
		}
